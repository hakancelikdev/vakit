...
**Link-preview cards** (`og:image`, 1200×630) are rendered per language from the page's own hero copy and that language's prayer-screen screenshot. They still show the pre-redesign serif type — re-run on a Mac to bring them in line. They are the one place web fonts are still used (Instrument Serif + Noto, `tools/fonts.js`) — the pages themselves use the system font. Re-run `make-og.js` after changing `h1a/h1b/h1c`/`eyebrow` or re-importing screenshots; `build.js` fails if a card is missing.
...
**Shown exactly as in the app — the app's wording wins** (owner's decision, 2026-09-26). That includes the "correct" principle's "we are still working to close the last minute of difference": it is the app's honest statement of today's gap, not a site promise, so the no-future rule below does not apply to it. Change the manifesto in the app, never on the site.
...
`LANGS` in `content.js` is the single list: URL, direction (`rtl`), writing system (`script` → typography rules in `styles.css`; `tools/fonts.js` `SCRIPTS` for the link-preview cards), screenshot and video source and the clock's home city. Adding a language = a `LANGS` entry + `locales/<lang>.js` + its code in `docs/language-detection.js` (`npm test` checks the last one).
...
- **The hero, top to bottom:** the slogan (`eyebrow`, "İbadetin yarıda kesilmesin"), the headline — which opens the manifesto, marked by a filled arrow, no label — the line under it, the App Store button, the four promises as small print (`pillars`: free · ad-free · offline · no account) and three numbers (rating, methods, languages). Each fact is said once: the old "0 ₺" stat went when the promises arrived (2026-09-26).
- **The comparison** is a table on wide screens and one labelled card per row on phones (`data-label` on the cells), so the Vakit column is never scrolled off-screen.
